...
- The server snapshot endpoint reconstructs **gate state only** — the returned `deal`/`salesStage` are current values, not historical. So the historical Briefing recomputes technical progress + gate-driven risk posture as-of, but economics and sales stage stay current. Do not present financials/stage as historical. The "you are viewing the past" signal is the **sticky amber control bar** in `briefing-mode.tsx` (amber tint + `As of <date>` mono dateline + "Back to today"); the in-document amber banner that used to sit in `briefing-presentation.tsx` was removed as a duplicate of it. The bar must stay `sticky` — it is the only on-screen indicator, so if it scrolled away nothing would say the numbers are historical.
