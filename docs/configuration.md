...
| `SUPER_ADMIN_EMAIL` | — | — | Email address that receives initial admin grant (optional; the app creates no users on startup). Does **not** exempt the address from `ALLOWED_EMAIL_DOMAINS`. |
| `ALLOWED_EMAIL_DOMAINS` | — | `zohocorp.com` | Comma-separated email domains allowed to hold an account (`lib/email-domain.ts`). Matched exactly — subdomains need listing separately. Enforced both on invite (`POST /v1/users` → 400) and on first sign-in (an outside address is refused, not auto-provisioned as a reader). Existing accounts are never re-checked, so tightening this cannot lock out people who already signed in. A value that parses to nothing falls back to the default rather than allowing everything. **On a fresh install the first sign-in is also subject to this list** — if the intended first admin is off-domain, nobody can bootstrap the app. |
